...
<h3>Edit Data</h3>
  <hr>
              <!-- <div class="form-group">
                               <input type="hidden" value="<?php echo $data_paket->Id_paket ?>" name="id">
              </div> -->

              <div class="form-group">
                <label for="text">Jenis Paket</label>
                <select name="Jenis_paket" class="form-control">
                    <option value="Jenis_paket"><?php echo $data_paket->Jenis_paket ?></option>
                    <?php 
                ...
                <!-- <input type="text" name="Jenis_paket" value="<?php echo $data_paket->Jenis_paket ?>" class="form-control"> -->
                <input type="hidden" value="<?php echo $data_paket->Id_paket ?>" name="Id_paket">
              </div>

              <div class="form-group">
                <label for="text">Harga</label>
                <select name="Harga" class="form-control">
                    <option value="Harga">Rp <?php echo $data_paket->Harga ?></option>
                    <?php 
                    $sql = $this->db->get('jenis_paket');
                    foreach ($sql->result() as $row) {
                    ?>
                ...
                <!-- <input type="text" name="Harga" value="<?php echo $data_paket->Harga ?>" class="form-control"> -->
              </div>

              <div class="form-group">
                <label for="text">Fasilitas</label>
                <input type="text" name="Fasilitas" value="<?php echo $data_paket->Fasilitas ?>" class="form-control">
              </div>

              <div class="form-group">
                <label for="text">Kegiatan</label>
                <input type="text" name="Kegiatan" value="<?php echo $data_paket->Kegiatan ?>" class="form-control">
              </div>
              <button type="submit" class="btn btn-secondary btn-sm">Update</button>
              <a href="<?php echo base_url('data/')?>" class="btn btn-sm btn-success">Back</a>
            <?php echo form_close() ?>
        </div>
    </form>
    </div>
